...
with open(config['mirrors_table'], 'a') as f:
    print('| Name | Sponsor | HTTPS | HTTP | RSYNC |\n| --- | --- | --- | --- | --- |', file=f)
    for mirror in all_mirrors:
        for protocol in ['http', 'https', 'rsync']:
            try:
        ...
        print("|%s|[%s](%s)|%s|%s|%s|" % (mirror['name'],mirror['sponsor'],mirror['sponsor_url'],mirror['https_link'],mirror['http_link'],mirror['rsync_link']), file=f)
